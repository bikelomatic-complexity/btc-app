...
export class AddPointLocation extends Component {
  ...
  render() {
    ...
    const { services, alerts, tracks, mapState, setMapCenter, filters,
      settings, setGeoLocation, setMapZoom, setMapLoading} = this.props;
    ...
    return (
      <div className="hideZoomControl" style={layoutStyle}>
        <PointMap services={services} alerts={alerts} filters={filters}
                  mapState={mapState} tracks={tracks} settings={settings}
            afterMoved={this.updateLocation.bind(this)}
            setMapCenter={setMapCenter} setGeoLocation={setGeoLocation}
            setMapZoom={setMapZoom} setMapLoading={setMapLoading}
        />
